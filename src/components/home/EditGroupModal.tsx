import { updateGroup } from "../../services/networkServices/networkService";
...
import { Group } from "./DeletingModal";
...
const EditGroupModal: React.FC<EditGroupModalProps> = ({
  ...
  const [endDate, setEndDate] = useState("2024/04/03");
  ...
  const handleOnsubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    updateGroup(body)
      .then((res) => {
        if (res.success) {
          toast.success("Group muvaffaqqiyatli yangilandi!");
          onClose();
          redownloadGroupsList();
          setIsLoading(false);
        }
      ...
      .catch((err) => {
        ...
        setIsLoading(false);
        toast.error(
          "Group yangilashda xatolik sodir bo'ldi, iltimos boshqattan harakat qiling!"
        );
      });
  };
  ...
  return (
    <form onSubmit={handleOnsubmit}>
      <h3 className="text-green-500 font-bold text-xl">Updating Group</h3>
      <div className="mt-3">
        <label
          htmlFor="group_name"
